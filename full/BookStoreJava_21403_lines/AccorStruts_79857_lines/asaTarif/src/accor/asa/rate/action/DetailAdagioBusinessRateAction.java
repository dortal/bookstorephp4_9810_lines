...
import com.accor.asa.rate.form.AdagioBusinessRateFormBean;
...
@Results({
    ...
    @Result(name = MainAction.RELOAD,   type = ServletActionRedirectResult.class,   value = "listAdagioBusinessRate.action", params = {"idPeriodeValidite", "${idPeriodeValidite}", "codeEcran", "${codeEcran}"})
})
public class DetailAdagioBusinessRateAction extends DetailHomeRateAction  {
	...
	protected void prepareAditionlDataForDisplayPage(Contexte contexte) throws RatesException {
		...
		if (form.getKey().equals("")) {
            ...
            form.setIdDureeSejour(defaultValuesProvider.getDefaultIdDureeSejour(codeRateLevel));
            // Default devise
	...
	private void checkFields() throws RatesUserException, RatesTechnicalException {
        AdagioBusinessRateFormBean form = (AdagioBusinessRateFormBean) getModel();
	...
	protected void completeRateData() throws RatesException {
        AdagioBusinessRateFormBean model = (AdagioBusinessRateFormBean)getModel();
