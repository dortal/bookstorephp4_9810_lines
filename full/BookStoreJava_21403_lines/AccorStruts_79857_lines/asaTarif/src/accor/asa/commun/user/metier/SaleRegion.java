...
public class SaleRegion extends Element {

	public static final int NO_CONSTRAINT_ERROR					= 0;
	public static final int SALEZONE_CONSTRAINT_ERROR			= 1;
	 ...
	 * @return
	 */
	public boolean notExist( final List<SaleRegion> regions ) {

		if( regions == null )
			return true;
		
		for( int i=0, size=regions.size(); i<size; i++ ) {
			if( StringUtils.equalsIgnoreCase( regions.get( i ).getCode(), code ) 
				|| StringUtils.equalsIgnoreCase( regions.get( i ).getLibelle(), libelle ) )
				return false;
		}
	 ...
	 * @return
	 */
	public boolean isUnique( final List<SaleRegion> regions ) {
		
		if( regions == null )
			return true;
		
		for( int i=0, size=regions.size(); i<size; i++ ) {
			if( StringUtils.equalsIgnoreCase( regions.get( i ).getLibelle(), libelle ) 
			&& ( ! ( StringUtils.equalsIgnoreCase( regions.get( i ).getCode(), code ) ) ) )
			return false;
		}
	...
	public static String getErrorMessage( final int codeError, final Contexte contexte ) throws TechnicalException {
		...
		switch( codeError ) {
			case SALEZONE_CONSTRAINT_ERROR :
				message.append(trans.getLibelle("ADMIN_USERS_SALESREGION_SALEZONE_CONSTRAINT_ERROR"));
				break;
			case USER_CONSTRAINT_ERROR :
				message.append(trans.getLibelle("ADMIN_USERS_SALESREGION_USER_CONSTRAINT_ERROR"));
				break;
			case DUPLICATE_SALEZONE_CONSTRAINT_ERROR : 
				message.append( trans.getLibelle( "ADMIN_USERS_SALESREGION_DUPLICATE_REGION" ) );
				break;
		}
