...
import com.accor.asa.commun.reference.metier.FacturationModeByTOCachList;
 ...
 * Time: 11:35:35
 */
public class FacturationModeByTORefDAO extends RefDAO {

    private static final String ADMIN_SELECT_PROC_NAME  = "ref_selFacturationModeByTO";
    private static final String INSERT_PROC_NAME        = "ref_addFacturationModeByTO";
    private static final String DELETE_PROC_NAME        = "ref_delFacturationModeByTO";
    ...
    /**
     * Retourne la liste des modes de facturation
     * par type offre
     *
     * @param context
     * @param bean
	...
	public List<ParamRefBean> getAdminRefList( Contexte contexte, RefBean bean ) throws TechnicalException {
        try {
            List<ElementWithState> listJointure = (List<ElementWithState>) 
            		SQLCallExecuter.getInstance().executeSelectProc(
                    ADMIN_SELECT_PROC_NAME,
                    new SQLParamDescriptorSet(),
                    "FacturationModeByTORefDAO", "getAdminRefList",
                    ...
                    new SQLResultSetReader() {
                        public Object instanciateFromLine(ResultSet rs) throws SQLException {
                            ...
                            obj.setCode(StringUtils.trimToEmpty(rs.getString("idFacturation")));
                            obj.setLibelle(StringUtils.trimToEmpty(rs.getString("libelle")));
            ...
            List<TypeOffreRefBean> listTO = (List<TypeOffreRefBean>) PoolCommunFactory.getInstance().getRefFacade()
            	.getRefList( RefFacade.TYPE_OFFRE_REF_KEY, contexte );
    ...
    /**
     * Update la liste des modes de facturation
     * par type offre
     *
     * @param context
     * @param bean
     ...
     */
    public int updateRef(Contexte context, RefBean bean)
            throws TechnicalException {
        ...
        try {
            ...
            SQLCallExecuter.getInstance().executeUpdate(
                    ...
                    "FacturationModeByTORefDAO",
                    "updateRef",
                    context.getContexteAppelDAO()
            );
            LogCommun.traceFonctionnelle(context.getCodeUtilisateur(), DELETE_PROC_NAME, "DEL", temp.getCode());
            // Add
            ElementWithState el;
            if (temp.getParams().size() > 0) {
                for (int i = 0, size = temp.getParams().size(); i < size; i++) {
                    ...
                    LogCommun.traceFonctionnelle(context.getCodeUtilisateur(), INSERT_PROC_NAME, "ADD", temp.getCode()+","+el.getCode());
                }
            }
            return 0;
        } catch (SQLException e) {
    ...
    protected String getCacheClassName () {
        return FacturationModeByTOCachList.class.getName();
    }

    protected CachableInterface getObjectInCache (String codeLangue) {
        return (CachableInterface) CacheManager.getInstance().getObjectInCache(FacturationModeByTOCachList.class);
    }

    @SuppressWarnings("unchecked")
	@Override
	protected CachableInterface createCacheObject(List<?> data, Contexte contexte) throws TechnicalException, IncoherenceException {
    	 return new FacturationModeByTOCachList((List<ParamRefBean>) data);
	}
