...
import com.accor.asa.commun.reference.metier.FacturationModeCachList;
import com.accor.asa.commun.reference.metier.FacturationModeRefBean;
 ...
 * Time: 15:15:53
 */
public class FacturationModeRefDAO extends RefDAO {

    private static final String SELECT_PROC_NAME        = "ref_selFacturationMode";
	private static final String ADMIN_SELECT_PROC_NAME  = "ref_selFacturationMode";
	private static final String INSERT_PROC_NAME        = "ref_addFacturationMode";
	private static final String UPDATE_PROC_NAME        = "ref_updFacturationMode";
	private static final String DELETE_PROC_NAME        = "ref_delFacturationMode";
	...
	protected SQLParamDescriptorSet getProcParameters (int type, RefBean bean, String codeLangue) {
		FacturationModeRefBean temp = (FacturationModeRefBean) bean;
		switch (type) {
			case SELECT :
			...
			case ADMIN_SELECT :
				return new SQLResultSetReader() {
					public Object instanciateFromLine(ResultSet rs) throws TechnicalException, SQLException {
						FacturationModeRefBean bean = new FacturationModeRefBean();
    ...
    protected String getCacheClassName () {
        return FacturationModeCachList.class.getName();
    }

    protected CachableInterface getObjectInCache (String codeLangue) {
        return (CachableInterface) CacheManager.getInstance().getObjectInCache(FacturationModeCachList.class, codeLangue);
    }
	...
	protected CachableInterface createCacheObject(List<?> data, Contexte contexte) throws TechnicalException, IncoherenceException {
    	return new FacturationModeCachList((List<FacturationModeRefBean>) data, contexte.getCodeLangue());
	}
